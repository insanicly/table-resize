(function () {
    ...
    MouseHandler.prototype = (function () {
        ...
        function _mousePrepareClick() {
            console.log('_mousePrepareClick');
        }

        function _mousePrepareDrag() {
            console.log('_mousePrepareDrag');
        }
        ...
        function _mouseExecuteClick() {
            console.log('_mouseExecuteClick');
        }
    ...
    function TableResize(table, options) {
        ...
        for (var i = 0; i < ((options.fixed) ? (length - 1) : length); i++) {
            ...
            var resizeTextWrapper = document.createElement('div');
            resizeTextWrapper.className = "resize-text";
            for (var j = 0; j < cell.children.length; j++) {
                var removedChild = cell.removeChild(cell.children[j])
                resizeTextWrapper.appendChild(removedChild);
            }


            var resizeElem = document.createElement('div');
            resizeElem.className = "resize-elem";

            var resizeBaseWrapper = document.createElement('div');
            resizeBaseWrapper.className = "resize-base";
            resizeBaseWrapper.appendChild(resizeElem);
            resizeBaseWrapper.appendChild(resizeTextWrapper);
            if (cell.childNodes.length > 0) {
                cell.removeChild(cell.childNodes[0]);
            }

            cell.appendChild(resizeBaseWrapper);

            addEvent(cell.childNodes[0].childNodes[0], 'mousedown', function (event) {
                resizeHandler._mouseDown(event);
            });

            addEvent(cell.childNodes[0].childNodes[0], 'click', function (event) {
